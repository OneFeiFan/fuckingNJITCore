...
class DecisionEngine {
    ...
    fun calculateTargetSleepTime(
        ...
        // 理想睡眠 8小时，严重缺觉线 5.5小时
        // 当睡眠下降到 5.5 小时，因子直接拉满到 1.0
        val idealSleepMins = DecisionConfig.BASE_SLEEP_HOURS * 60//期望睡眠阈值
        val severeLackMins = DecisionConfig.SEVERE_SLEEP_LACK_HOURS * 60 // 最小睡眠阈值
        val sleepDebtFactor =
        ...
        // 体能消耗因子 [0.0 ~ 1.0]
        // 以 BASE_STEPS(7000) 为归一化上限，SEDENTARY_STEPS(5000) 为生理中性轴
        ...
        // 运动小于5000步不施加正向偏移，
        // 运动大于5000步按比例线性提前，达到 BASE_STEPS 时最大提前 30 分钟
        val fatigueOffset = if (fatigueRatio <= neutralRatio) {
            0f
        } else {
    ...
    private fun calculatePhysicalScore(recentSleepRecords: List<DailyRecord>, steps: Int): Float {
        ...
        val avgSleepMins = recentSleepRecords.map { it.totalSleepMinutes }.average() // 多日平均睡眠时长
        val avgSleepHours = (avgSleepMins / 60.0).toFloat()
        val lastNightSleepHours =
            (recentSleepRecords.last().totalSleepMinutes / 60.0).toFloat() // 昨夜睡眠时长
